...
{
    WebRTCStatGroup* stats = [self getSessionStats:sessionId];
    [stats startMetric:statName];
    [self saveStats:sessionId _stats:stats];
}
...
-(WebRTCStatGroup*)getSessionStats:(NSString *)sessionId
{
    ...
    if([sessions objectForKey:sessionId])
    {
        stats = [sessions objectForKey:sessionId];
    }
    else
    {
...
{
    [sessions setObject:stats forKey:sessionId];
}
...
-(void)notifyApplication:(NSString*)key _value:(id)value
{
    ...
    [self.delegate onUpdateStats:key _statValue:value];
