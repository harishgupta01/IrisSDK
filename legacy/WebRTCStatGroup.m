...
-(void)startMetric:(NSString*)statName
{
    ...
    [self writeStat:statName _key:@"startTime" _value:[isoDateFormatter stringFromDate:now]];
}
...
-(void)stopMetric:(NSString*)statName
{
    ...
    if (from != nil)
    {
        [self writeStat:statName _key:@"stopTime" _value:[isoDateFormatter stringFromDate:now]];
    }
...
-(void)storeReaccuring:(NSString*)statName _values:(NSDictionary*)values
{
    ...
    {
        NSDate* now = [NSDate date];
        [values setValue:[isoDateFormatter stringFromDate:now] forKey:@"timestamp"];
...
//        NSDate* stopTime = [dateFormatter dateFromString:[stat objectForKey:@"stopTime"]];
        ...
        NSDate* startTime = [isoDateFormatter dateFromString:[stat objectForKey:@"startTime"]];
        NSDate* stopTime = [isoDateFormatter dateFromString:[stat objectForKey:@"stopTime"]];
        ...
        [streamInfo setObject:[stat objectForKey:@"startTime"] forKey:@"startTime"];
        [streamInfo setObject:[stat objectForKey:@"stopTime"] forKey:@"stopTime"];
