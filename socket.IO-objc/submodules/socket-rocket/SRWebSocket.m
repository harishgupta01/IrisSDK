...
- (void)didConnect
{
    ...
    int RetValue = SecRandomCopyBytes(kSecRandomDefault, keyBytes.length, keyBytes.mutableBytes);
    SRFastLog(@"SecRandomCopyBytes %d", RetValue);
    ...
    IRISLogInfo(@"Check if we have a cookie for this server");
...
- (void)handlePong
{
    [self _performDelegateBlock:^{
        if ([self.delegate respondsToSelector:@selector(webSocketDidOpen:)]) {
            [self.delegate webSocket:self didReceivePong:NULL];
        };
    }];
}

- (void)handlePong:(NSData *)pongData {
    IRISLogInfo(@"IrisRtcSdk::Received pong");
...
- (void)_sendFrameWithOpcode:(SROpCode)opcode data:(id)data;
{
    ...
    if (!useMask) {
        ...
        int RetValue = SecRandomCopyBytes(kSecRandomDefault, sizeof(uint32_t), (uint8_t *)mask_key);
        SRFastLog(@"SecRandomCopyBytes %d", RetValue);
...
static inline void SRFastLog(NSString *format, ...)  {
    ...
    IRISLogInfo(@"[SR] %@", formattedString);
#endif
}
...
- (void)main;
{
    @autoreleasepool {
        ...
        NSTimer *timer = [[NSTimer alloc] initWithFireDate:[NSDate distantFuture] interval:0.0 target:nil selector:nil userInfo:(id)userinfo repeats:NO];
