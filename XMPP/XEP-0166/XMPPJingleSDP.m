...
@interface XMPPJingleSDPUtil(){
    
}

@property (nonatomic,copy) NSMutableDictionary *streamRoutingIdDict;
...
-(id)init{
     self = [super init];
    _streamRoutingIdDict = [[NSMutableDictionary alloc]init];
    return self;
}
- (NSString *)parseGroups:(XMPPIQ *)iq
{
   ...
   /* NSArray *groups = [[iq elementForName:@"jingle"] elementForName:@"group"];
    if (groups != nil)
    {
        IRISLogError(@"Error in retrieving groups");
    }
    ...
    if (jElement == nil)
    {
        IRISLogInfo(@"XMPP Parsing: No Jingle element");
    ...
    if (group == nil)
    {
        IRISLogInfo(@"XMPP Parsing: No group element");
        return NULL;
    }
...
-(NSString*)routingIdFor:(NSString*)streamId{
    return [_streamRoutingIdDict objectForKey:streamId];
}
- (NSString *)parseMedia:(NSXMLElement *)content
{
    ...
    //muc changes
    NSString *data = @"data";
    if([data isEqualToString:(NSString *)[content attributeStringValueForName:@"name" ]])
    {
        [contentString appendFormat:@"m=application 9 DTLS/SCTP 5000"];
    }
    else
    ...
    if (transportInfo)
    {
            ...
            for (int i=0; i < [candidates count]; i++)
            {
                ...
                if (canElement)
                {
                    ...
                    if ([type isEqualToString:@"srflx"] || [type isEqualToString:@"prflx"] || [type isEqualToString:@"relay"]) {
                        if ([canElement attributeStringValueForName:@"rel-addr"] && [canElement attributeStringValueForName:@"rel-port"]) {
                            [sdp addObject:@"raddr"];
        ...
        if(sctpmap)
        {
            NSString *port = [sctpmap attributeStringValueForName:@"port"];
            if(port == nil){
            port = [sctpmap attributeStringValueForName:@"number"];
            }
            NSString *protocol = [sctpmap attributeStringValueForName:@"protocol"];
            NSString *streams = [sctpmap attributeStringValueForName:@"streams"];
            [contentString appendFormat:@"a=sctpmap:%@ %@ %@\r\n", port,protocol,streams];
        }
    ...
    {
        // TODO : count <= 1
        //Girish:: Adding ssrc to sdp to get Audio and video stats for p2p call
        //  if(![XMPPWorker sharedInstance].isVideoBridgeEnable){
        //if ([sources count] > 1)
        //	{
            for (int i=0; i < [sources count]; i++)
            {
                ...
                if([source elementForName:@"ssrc-info"] != NULL){
                    
                    NSXMLElement *ssrcinfo = [source elementForName:@"ssrc-info"];
                    
                    NSString *owner = [ssrcinfo attributeStringValueForName:@"owner"];
                    
                    targetJID = [XMPPJID jidWithString:owner];
                ...
                for (int k=0; k< [parameters count]; k++)
                {
                    ...
                    if (name && value)
                    {
                        if([targetJID isFullWithUser]){
                           
                            if([name isEqualToString:@"msid"]){
                                [_streamRoutingIdDict setObject:[targetJID resource] forKey:[value componentsSeparatedByString:@" "][0]];
    ...
    //IRISLogInfo(@"ContentString is %@", contentString);
...
- (NSString *)XMPPToSDP:(XMPPIQ *)iq
{
    ...
    mediaContent=[[NSMutableDictionary alloc]init]; // initiate initially
    [mediaContent removeAllObjects];
    ...
    for (int i=0; i<[mediaContents count]; i++)
    {
        ...
        NSArray *header =[[NSArray alloc]initWithObjects:mediaStr, nil];
        //        NSDictionary *format=[[NSDictionary alloc]initWithObjectsAndKeys:header,contentname, nil];
        [mediaContent setObject:header forKey:contentname];
    ...
    //IRISLogInfo(@"saved headers %@",mediaContent);

    //IRISLogInfo (@"SDP is %@", SDP);
    return SDP;
}
...
- (NSString *)modifySDP:(XMPPIQ *)iq
{
    ...
    //datachannel-changes
    if ([mediaContent allKeys].count==3)
    {
    ...
    for (int i=0; i<[mediaContents count]; i++)
    {
        ...
        if([contentname isEqualToString:@"data"]){
            continue;
        }
            
        NSMutableArray *tempObjects=[[mediaContent objectForKey:contentname] mutableCopy];
        for (int c=0; c<tempObjects.count; c++)
        {
            NSString *splitstr = [NSString stringWithFormat:@"%@",[tempObjects objectAtIndex:c]];
            
              //Girish :: commented below lines to add session-initate ssrc to sdp.
//            NSArray *splitarr = [splitstr componentsSeparatedByString:@"a=ssrc"];
//            
//            [contentString appendFormat:@"%@",[splitarr objectAtIndex:0]];
            
            [contentString appendFormat:@"%@",splitstr];
        ...
        if (sources.count>0)
        {
            NSMutableString *finalContentString = [[NSMutableString alloc]init];
            for (int i=0; i < [sources count]; i++)
            {
                ...
                if ([tempContentString rangeOfString:@"msid"].location != NSNotFound) {
                    [finalContentString appendFormat:@"%@",tempContentString];
                }
            }
            [tempObjects addObject:finalContentString];
            [mediaContent setObject:tempObjects forKey:contentname];
    ...
    if ( mediaContent.count==3)
    {
        NSMutableArray *tempObjects=[[mediaContent objectForKey:@"data"] mutableCopy];
        for (int c=0; c<tempObjects.count; c++)
        {
            [SDP appendFormat:@"%@",[tempObjects objectAtIndex:c]];
        }
    }
...
- (NSString *)modifySourceRemoveSDP:(XMPPIQ *)iq
{
    ...
    {        
        ...
        if (sources.count>0)
        {
            for (int i=0; i < [sources count]; i++)
            {
                ...
                for (int m=0; m<tempObjects.count; m++)
                {
                    ...
                    NSString *tempstring = tempObjects[m];
                    NSArray *brokenByLines=[tempstring componentsSeparatedByString:@"\r\n"];
                        
                    for (id object in brokenByLines) {
                        
                         NSString *tempstring = object;
                         if([tempstring length]>0 && ![tempstring containsString:ssrcValue] ){
                                [finalstring appendFormat:@"%@\r\n",tempstring];
                         }
                    }
                        
                    [tempObjects setObject:finalstring atIndexedSubscript:m];
        ...
        for (int c=0; c<tempObjects.count; c++)
        {
            [contentString appendFormat:@"%@",[tempObjects objectAtIndex:c]];
        }
       
        [mediaContent setObject:tempObjects forKey:contentname];
    
    }
    if (mediaContents.count==3)
    {
        NSMutableArray *tempObjects=[[mediaContent objectForKey:@"data"] mutableCopy];
        for (int c=0; c<tempObjects.count; c++)
        {
...
- (XMPPIQ *)SDPToXMPP:(NSString *)sdp action:(NSString *)action initiator:(XMPPJID *)initiator target:(XMPPJID *)target UID:(NSString *)UID SID:(NSString *)SID
{
    ...
    for (int i=0; i < [contents count]; i++)
    {
        ...
        if([[content objectForKey:@"creator"] isEqualToString:@"initiator"] && [[content objectForKey:@"senders"] isEqualToString:@"initiator"]){
            creator = @"responder";
            sender = @"initiator";
        }
        
        if([[content objectForKey:@"creator"] isEqualToString:@"initiator"] && [[content objectForKey:@"senders"] isEqualToString:@"responder"]){
            creator = @"responder";
            sender = @"responder";
        }
        ...
        for (int j=0; j < [payloads count]; j++)
        {
            ...
            NSArray *parameters = [payload  objectForKey:@"parameter"];
            for (int k=0; k< [parameters count]; k++)
            {
        ...
        if (ssrc)
        {
            ...
            for (int s=0; s< [sources count]; s++)
            {
                ...
                NSArray *parameters = [source objectForKey:@"parameter"];
        ...
        //muc changes
        if([transport objectForKey:@"sctp"] != nil)
        {
            NSArray* sctp = [transport objectForKey:@"sctp"];
            
            for (int i=0; i < [sctp count]; i++)
            {
                NSXMLElement *sctpmap = [NSXMLElement elementWithName:@"sctpmap"];
                [sctpmap addAttributeWithName:@"xmlns" stringValue:@"urn:xmpp:jingle:transports:dtls-sctp:1"];
                [sctpmap addAttributeWithName:@"port" stringValue:[sctp[i] objectForKey:@"number"]];
                [sctpmap addAttributeWithName:@"number" stringValue:[sctp[i] objectForKey:@"number"]];
                [sctpmap addAttributeWithName:@"protocol" stringValue:[sctp[i] objectForKey:@"protocol"]];
                [sctpmap addAttributeWithName:@"streams" stringValue:[sctp[i] objectForKey:@"streams"]];
                
                [transElement addChild:sctpmap];
                if(sdpFprElement)
...
- (XMPPIQ *)CandidateToXMPP:(NSDictionary *)dict action:(NSString *)action initiator:(XMPPJID *)initiator target:(XMPPJID *)target UID:(NSString *)UID SID:(NSString *)SID
{
    ...
    // TBD : To find a way to parse the SDP data and convert the same to XMPP
    // Parse xmpp
    IRISLogInfo(@"Before converting candidate %@", [dict objectForKey:@"candidate"]);
    NSDictionary* candidate = [JAHConvertSDP candidateForLine:[dict objectForKey:@"candidate"]]; // or @"responder"
    IRISLogInfo(@"after converting candidate %@", candidate);
    ...
    if ( (id != nil) && ([id length] > 0))
    {
     ...
     //   [contentElement addAttributeWithName:@"creator" stringValue:@"initiator"];
        ...
        NSString *network = [candidate objectForKey:@"network"];
        NSString *did = [candidate objectForKey:@"id"];
        if([candidate objectForKey:@"tcptype"])
        {
            [canElement addAttributeWithName:@"tcptype" stringValue:[candidate objectForKey:@"tcptype"]];
        }
        ...
        if(![type isEqualToString:@"host"]){
            [canElement addAttributeWithName:@"rel-port" stringValue:[candidate objectForKey:@"rel-port"]];
            [canElement addAttributeWithName:@"rel-addr" stringValue:[candidate objectForKey:@"rel-addr"]];
...
- (void) splitSDP:(NSString*)sdp
{
    ...
    IRISLogInfo(@"Media%@", media);
    IRISLogInfo(@"Session%@", session);
    ...
    if (sdp != nil)
    {
        ...
        for (int i=0; i<[mediaContents count]; i++)
        {
            ...
            if (mediaStr)
            {
                ...
                [elem addAttributeWithName:@"direction" stringValue:direction];
