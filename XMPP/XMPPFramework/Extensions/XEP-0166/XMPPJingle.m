...
- (BOOL)sendSessionMsg:(NSString *)type  data:(NSDictionary *)data target:(XMPPJID *)target
{
    ...
    if (sdp != nil)
    {
        ...
        [miscData addAttributeWithName:@"event" stringValue:_event];
        [miscData addAttributeWithName:@"traceid" stringValue:_traceId];
        [miscData addAttributeWithName:@"nodeid" stringValue:_nodeId];
        [miscData addAttributeWithName:@"cnodeid" stringValue:_cnodeId];
        [miscData addAttributeWithName:@"unodeid" stringValue:_unodeId];
        [sdp addChild:miscData];
...
- (void)xmppStream:(XMPPStream *)sender didReceivePresence:(XMPPPresence *)presence
{
    ...
    if([type isEqualToString:@"error"] && [errorelement attributeIntValueForName:@"code"] == 403){
        ...
        [details setValue:@"Required participants has already joined, So not allowed to join the room" forKey:NSLocalizedDescriptionKey];
        ...
        if([type isEqualToString:@"error"] && [errorelement attributeIntValueForName:@"code"] == 409){
            ...
            NSMutableDictionary* details = [NSMutableDictionary dictionary];
            [details setValue:@"Room has already created, So not allowed to create the room" forKey:NSLocalizedDescriptionKey];
