...
static NSString* kInsecureSocketPortURL =@"ws://%@/xmpp-websocket/routingid/%@/timestamp/%@/token/%@";
static NSString* kSecureSocketPortURL =@"wss://%@/xmpp-websocket/routingid/%@/timestamp/%@/token/%@";
...
@interface IrisXMPPStream ()<SRWebSocketDelegate>
{
    ...
    GCDMulticastDelegate <IrisXMPPStreamDelegate,IrisXMPPSessionDelegate> *multicastDelegate;
    ...
    NSString* nodeId;
    NSString* cnodeId;
    NSString* maxParticipants;
    NSString* unodeId;
    BOOL IsXMPPRoomCreator;
...
@implementation IrisXMPPStream

@synthesize tag = userTag,actualHostName,dataElement;
...
-(void) webSocket:(SRWebSocket *)webSocket didReceivePong:(NSData *)pongData{
    [multicastDelegate xmppStream:self onPongMessage:NULL];
}

- (void) webSocketDidOpen:(SRWebSocket *)webSocket
{
    dispatch_sync(xmppQueue, ^{
        IRISLogInfo(@"Socket opened.");
        [multicastDelegate onXmppServerConnected];
...
- (void)startIQACKTimer:(NSString *)elementId{
    
    
    dispatch_async(dispatch_get_main_queue(), ^{
        IRISLogVerbose(@"_iqAckTimer:: start Timer");
        NSMutableDictionary *info = [[NSMutableDictionary alloc] init];
        if(elementId != nil)
        [info setObject:elementId forKey:@"StanzaId"];
        _iqAckTimer = [NSTimer scheduledTimerWithTimeInterval:5
                                                          target:self
                                                        selector:@selector(reportSessionError:)
                                                        userInfo:info
                                                         repeats:NO
                          ];
    });
...
-(void)reportSessionError:(NSTimer *)timer
{
    hasRecvIQAck = true;
    [self processIQArray];
    
    if(multicastDelegate != nil){
         IRISLogVerbose(@"reportSessionError:: Did not receive ACK");
        NSMutableDictionary* details = [NSMutableDictionary dictionary];
        [details setValue:@"Did not receive ACK for the IQ" forKey:NSLocalizedDescriptionKey];
        NSError *error = [NSError errorWithDomain:@"" code:ERR_XMPP_NOACK userInfo:details];
        
        NSDictionary *dict = [timer userInfo];
        [multicastDelegate xmppStream:self onSessionError:error additionalInfo:dict];
    }
}

-(void)resetIQTimer:(NSString *)elementId{
    IRISLogVerbose(@"_iqAckTimer:: resetIQTimer");
    [_iqAckTimer invalidate];
    [self startIQACKTimer:elementId];
}

- (void)_timerHeartBeat:(NSTimer *)timer{
    
    IRISLogVerbose(@"Webrtc:Session:: _timerHeartBeat");
    
    NSString *termStr = @"HeartBeat";
    NSData *termData = [termStr dataUsingEncoding:NSUTF8StringEncoding];
    
    IRISLogInfo(@"SEND: %@", termStr);
...
{
    IRISLogError(@"Socket failed with error ... %@", [error localizedDescription]);
    // Assuming this resulted in a disconnect
    //[delegate onDisconnect:error];
    [multicastDelegate xmppStream:self onError:error];
}
...
{
    IRISLogInfo(@"Socket closed. %@", reason);
    IRISLogInfo(@"Disconnecting from socket 777");
    state = STATE_XMPP_DISCONNECTED;
    [multicastDelegate xmppStreamDidDisconnect:self withError:nil];
}

+(BOOL)isPeriodicPresence:(XMPPPresence*)presence
{
    ...
    if(data != nil)
    {
        ...
        if(type != nil)
        {
            if([type isEqualToString:@"periodic"])
            {
                return YES;
            }
        }
...
- (void)commonInit
{
    ...
    IRISLogInfo(@"Disconnecting from socket 888");
    state = STATE_XMPP_DISCONNECTED;
...
- (id)init
{
    IRISLogVerbose(@"IrisXMPPStream::Init");
    //if ((self = [super init]))
    //{
    ...
    iqArray = [[NSMutableArray alloc]init];
    hasRecvIQAck = true;
        IRISLogInfo(@"Initializing resourceId");
        resourceId = [self generateUUID];
   // }
    return self;
}
...
- (XMPPStreamState)state
{
    IRISLogWarn(@"Disconnecting from socket 999");
...
- (void)setActualHostName:(NSString *)newHostName
{
    if (dispatch_get_specific(xmppQueueTag))
    {
        if (actualHostName != newHostName)
        {
            actualHostName = [newHostName copy];
        }
    }
    else
    {
        ...
        dispatch_async(xmppQueue, ^{
            actualHostName = newHostNameCopy;
        });
...
- (void)setIsXMPPRoomCreator:(BOOL)isInitiator
{
    dispatch_block_t block = ^{
        IsXMPPRoomCreator = isInitiator;
    };
...
- (void)addDelegate:(id)delegate delegateQueue:(dispatch_queue_t)delegateQueue
{
    // Asynchronous operation (if outside xmppQueue)
     IRISLogVerbose(@"IrisXMPPStream::addDelegate");
    hasRecvIQAck = true;
    [iqArray removeAllObjects];
    dispatch_block_t block = ^{
...
- (void)doConnectTimeout
{
    ...
    if (state != STATE_XMPP_DISCONNECTED)
    {
        ...
        if (state == STATE_XMPP_RESOLVING_SRV)
        {
            [srvResolver stop];
            srvResolver = nil;
            IRISLogWarn(@"Disconnecting from socket 1212");
            state = STATE_XMPP_DISCONNECTED;
        }
        else
        {
...
- (BOOL)connectToHost:(NSString *)host onPort:(UInt16)port withTimeout:(NSTimeInterval)timeout error:(NSError **)errPtr
{
    ...
    //urlStr = [NSString stringWithFormat:format, host, port];
    //urlStr = [NSString stringWithFormat:format,host,routingId,mucId,timestamp,token,traceId];
    urlStr = [NSString stringWithFormat:format,host,routingId,timestamp,token];
    
    //urlStr = @"wss://st-xmpp-cmce-002.poc.sys.comcast.net/xmpp-websocket";
...
- (BOOL)connectWithTimeout:(NSTimeInterval)timeout error:(NSError **)errPtr
{
    ...
    dispatch_block_t block = ^{ @autoreleasepool {
        ...
        else
        {
            ...
            result = [self connectToHost:actualHostName onPort:hostPort withTimeout:XMPPStreamTimeoutNone error:&connectErr];
            
            if (!result)
            {
                err = connectErr;
                IRISLogWarn(@"Disconnecting from socket 1313");
                state = STATE_XMPP_DISCONNECTED;
            }
        }
...
- (BOOL)connectTo:(XMPPJID *)jid withAddress:(NSData *)remoteAddr withTimeout:(NSTimeInterval)timeout error:(NSError **)errPtr
{
    ...
    dispatch_block_t block = ^{ @autoreleasepool {
        ...
        {
            err = connectErr;
            IRISLogWarn(@"Disconnecting from socket 1414");
            state = STATE_XMPP_DISCONNECTED;
        }
    ...
    dispatch_block_t block = ^{ @autoreleasepool {
        ...
        if (state != STATE_XMPP_DISCONNECTED)
        {
            ...
            if (state == STATE_XMPP_RESOLVING_SRV)
            {
                [srvResolver stop];
                srvResolver = nil;
                IRISLogWarn(@"Disconnecting from socket 111");
                state = STATE_XMPP_DISCONNECTED;
            ...
            else
            {
                //[asyncSocket disconnect];
                [_webSocket close];
                [self setDidStartNegotiation:NO];
                [self setIsAuthenticated:false];
                state = STATE_XMPP_DISCONNECTED;
                IRISLogWarn(@"Disconnecting from socket 222");
                // Everthing will be handled in socketDidDisconnect:withError:
            }
        }
    }};
    ...
    dispatch_block_t block = ^{ @autoreleasepool {
        ...
        if (state != STATE_XMPP_DISCONNECTED)
        {
            ...
            if (state == STATE_XMPP_RESOLVING_SRV)
            {
                [srvResolver stop];
                srvResolver = nil;
                IRISLogWarn(@"Disconnecting from socket 333");
                state = STATE_XMPP_DISCONNECTED;
...
- (BOOL)authenticateWithPassword:(NSString *)inPassword error:(NSError **)errPtr
{
    ...
    dispatch_block_t block = ^{ @autoreleasepool {
        ...
        {
            someAuth = [[IrisXMPPAnonymousAuthentication alloc] initWithStream:self password:password routingID:routingId traceID:traceId];
            result = [self authenticate:someAuth error:&err];
        }
...
- (void)sendPresence:(XMPPPresence *)presence withTag:(long)tag
{
    ...
    NSString* iqStr = [presence compactXMLString];
    if(![iqStr containsString:@"to="] && isFirstPresence)
    {
...
- (void)continueSendMessage:(XMPPMessage *)message withTag:(long)tag
{
    ...
    NSString* iqStr = [message compactXMLString];
...
-(void) processIQArray
{
    dispatch_async(dispatch_get_main_queue(), ^{
       ...
       //IRISLogVerbose(@"processIQArray::processing" );
        if (iqArray == nil || [iqArray count] == 0) {
            //IRISLogVerbose(@"processIQArray::processing11" );
            
            return;
        }
        //IRISLogVerbose(@"processIQArray::processing22" );
        [self sendElement:[iqArray queuePop]];
    });
...
{
   // IRISLogInfo(@"IrisXMPPStream::sendElement = %@",[element stringValue]);
    if ([element isKindOfClass:[XMPPIQ class]] ){
        XMPPIQ* iq = (XMPPIQ*) element;
        NSXMLElement *data = [iq elementForName:@"data" xmlns:@"urn:xmpp:comcast:info"];
        if(data != nil) {
            NSString *csevent = [[data attributeForName:@"event"] stringValue];
            if (csevent != nil && [csevent isEqualToString:@"callstats"]) {
                if(![iq isResultIQ] && ![iq isErrorIQ]){
                    if(!hasRecvIQAck){
                        [iqArray queuePush:element];
                       IRISLogVerbose(@"sendElement:: Locking11");
                        return;
                    }
                    hasRecvIQAck = false;
                    IRISLogVerbose(@"sendElement:: Locking22");
                    [self resetIQTimer:[[element attributeForName:@"id"] stringValue]];
    ...
    if(dataElement != nil && [ element elementForName:@"data" xmlns:@"urn:xmpp:comcast:info"] == nil && ![[[element attributeForName:@"id"] stringValue]  isEqual: @"c2s"]){
        [element addChild:[dataElement full]];
    }
    dispatch_block_t block = ^{ @autoreleasepool {
        //IRISLogInfo(@"XMPPSTREAM::sendElement = %ld",(long)state);
        if (state == STATE_XMPP_CONNECTED)
        {
...
/**
 * This method handles sending an IQ XML stanza.
 * If the XMPPStream is not connected, this method does nothing.
 *
 * After the element has been successfully sent,
 * This method will wait till it gets the receipt for the message sent.
 **/
...
{
    //IRISLogVerbose(@"continueReceiveIQ");
    if([iq isResultIQ]){
        
        //IRISLogVerbose(@"Sendelement:: Unlocking it");
        //IRISLogVerbose(@"_iqAckTimer:: invalidate");
        [_iqAckTimer invalidate];
        [self processIQArray];
    }
...
{
    XMPPLogVerbose(@"XMPP: Rx %@", element.description);
    if (element == nil) return;
...
- (void)handleStreamFeatures
{
    ...
    else if (![self isSecure] && [self startTLSPolicy] == XMPPStreamStartTLSPolicyRequired)
    {
        ...
        IRISLogInfo(@"Disconnect call from handleStreamFeatures");
        [self disconnect];
...
- (void)handleStartTLSResponse:(NSXMLElement *)response
{
    ...
    if ( ![[response name] isEqualToString:@"proceed"])
    {
        ...
        IRISLogInfo(@"Disconnect call from handleStartTLSResponse");
        [self disconnect];
...
- (void)startStandardBinding
{
    ...
    if ([requestedResource length] > 0)
    {
        ...
        [resource setStringValue:resourceId];
...
- (void)tryNextSrvResult
{
    ...
    {
        [self endConnectTimeout];
        IRISLogWarn(@"Disconnecting from socket 444");
        state = STATE_XMPP_DISCONNECTED;
...
- (void)socketDidDisconnect:(GCDAsyncSocket *)sock withError:(NSError *)err
{
    ...
    {
        IRISLogWarn(@"Disconnecting from socket 666");
        // Update state
        state = STATE_XMPP_DISCONNECTED;
...
- (void)xmppParser:(XMPPParser *)sender didReadRoot:(NSXMLElement *)root
{
    ...
    XMPPLogTrace();
    XMPPLogVerbose(@"RECV: %@", [root compactXMLString]);
...
- (void)xmppParser:(XMPPParser *)sender didReadElement:(NSXMLElement *)element
{
    ...
    XMPPLogVerbose(@"didReadElement: %@", [element compactXMLString]);
