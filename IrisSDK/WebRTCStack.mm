...
#define LIBSDK_VERSION "2.0.1.14" // Need to find a better way of doing this.
...
@interface WebRTCStack() <SignalHandlerDelegate>
{
    SignalHandler *sh;
...
- (void) createXMPPConnection:(NSString*)mucid _timestamp:(NSString*)timestamp _xmppToken:(NSString*)xmppToken _requestType:(NSString *)requestType
{
    ...
    _roomID = lines[0];
    __sessionConfig.rtcgSessionId = _roomID;
    ...
    [[XMPPWorker sharedInstance] setMucId:_roomID];
    ...
    [_session serverUrl:websocketURL routingId:stackConfig.routingId serviceId:serviceId];
}
