...
- (int)muteAudio
{
    ...
    for (RTCMediaStreamTrack *track in lms.audioTracks)
    {
        ...
        if (![track.label compare:@"ARDAMSa0"])  {
            ...
            {
                LogInfo(@" muteAudio, disable audio in recording !!!");
                [lms disableAudioRecording];
            }
            return 0;
        }
    }
...
- (int)unmuteAudio
{
    ...
    for (RTCMediaStreamTrack *track in lms.audioTracks)
    {
        if (![[track label] compare:@"ARDAMSa0"])  {
            ...
            if (_isRecordingEnabled)
            {
                LogInfo(@" muteAudio, enable audio in recording !!!");
                [lms enableAudioRecording];
            }
            return 0;
        }
    }
