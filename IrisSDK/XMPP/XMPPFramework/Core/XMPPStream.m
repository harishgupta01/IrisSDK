...
static NSString* kInsecureSocketPortURL =@"ws://%@/xmpp-websocket/routingid/%@/mucid/%@/timestamp/%@/token/%@/traceid/%@";
static NSString* kSecureSocketPortURL =@"wss://%@/xmpp-websocket/routingid/%@/mucid/%@/timestamp/%@/token/%@/traceid/%@";
...
- (void)setTraceId:(NSString *)newRoutingId
{
    if (dispatch_get_specific(xmppQueueTag))
    {
        if (traceId != newRoutingId)
        {
            traceId = [newRoutingId copy];
        }
    }
    else
    {
        NSString *newTraceIdCopy = [newRoutingId copy];
        
        dispatch_async(xmppQueue, ^{
            traceId = newTraceIdCopy;
        });
...
- (void)setCallType:(NSString *)type
{
    ...
    {
        NSString *newCallType = [type copy];
...
- (void)setMaxParticipants:(NSString *)participants
{
    ...
    {
        NSString *newId = [participants copy];
        
        dispatch_async(xmppQueue, ^{
            maxParticipants = newId;
        });
...
- (BOOL)registerWithElements:(NSArray *)elements error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
        ...
        NSString* iqStr = [iq compactXMLString];
        if(![iqStr containsString:@"urn:xmpp:comcast:info"])
        {
            NSXMLElement *miscData = [NSXMLElement elementWithName:@"data" xmlns:@"urn:xmpp:comcast:info"];
            //[miscData addAttributeWithName:@"event" stringValue:callType];
...
- (BOOL)authenticateWithPassword:(NSString *)inPassword error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
        ...
        {
            someAuth = [[XMPPAnonymousAuthentication alloc] initWithStream:self password:password routingID:routingId traceID:traceId];            result = [self authenticate:someAuth error:&err];
        }
...
- (void)continueSendElement:(NSXMLElement *)element withTag:(long)tag
{
    ...
    NSString* iqStr = [element compactXMLString];
    if(![iqStr containsString:@"urn:xmpp:comcast:info"])
    {
        NSXMLElement *miscData = [NSXMLElement elementWithName:@"data" xmlns:@"urn:xmpp:comcast:info"];
        //[miscData addAttributeWithName:@"event" stringValue:callType];
